...
public class AllEnquiries extends BaseActivity implements ChatItemClickListener, View.OnClickListener {
        ...
        StringRequest request = new StringRequest(Request.Method.POST, GlobalVariables.PREURL + GlobalVariables.GETCOUNSELLORDETAILBYPHONE, response -> {
            ...
            try {
                ...
                if (str_result.equalsIgnoreCase("true")) {
                    ...
                    AllEnquiries.this.user = new User();
...
//                setUserInfo();
    ...
    private void setUserInfoApi(JSONObject data_obj) {
        ...
        try {
            ...
            if (data_obj.getString("live_status").equalsIgnoreCase("Offline")) {
                sb.setChecked(false);
                userStatus.setText("Currently you are offline. Please change your status as your need");
            } else {
                sb.setChecked(true);
                userStatus.setText("Currently you are Online. You can change your status anytime");
            }

            if (data_obj.getString("image").equalsIgnoreCase("") || data_obj.getString("image").equalsIgnoreCase("null")) {
                Glide.with(this).load(user.getImage()).apply(new RequestOptions().placeholder(R.drawable.cousellers)).into(usersImage);
            } else {
    ...
    private void LoadServices() {
        ...
        StringRequest request = new StringRequest(Request.Method.POST, GlobalVariables.PREURL + GlobalVariables.GETCATEGORY, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                HomeAdapter mAdapter = new HomeAdapter(AllEnquiries.this, items);
                RecyclerView recyclerView = (RecyclerView) findViewById(R.id.rv_services);
                recyclerView.setLayoutManager(new GridLayoutManager(AllEnquiries.this, 2));
                recyclerView.setHasFixedSize(true);
                recyclerView.setAdapter(mAdapter);
                mAdapter.setOnItemClickListener(new HomeAdapter.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, CategoryModel obj, int position) {
                        Intent intent = new Intent(AllEnquiries.this, ServiceActivity.class);
        ...
        if (counsellerList.size() == 0) {
            ...
            StringRequest request = new StringRequest(Request.Method.POST, GlobalVariables.PREURL + GlobalVariables.GETCONSELLORLIST, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    ...
                    recyclerView.setLayoutManager(new LinearLayoutManager(AllEnquiries.this, LinearLayoutManager.HORIZONTAL, false));
                    recyclerView.setHasFixedSize(true);
            ...
            if (firebaseUser != null) {
                firebaseUser.getIdToken(false).addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        FetchMyUsersService.startMyUsersService(AllEnquiries.this, userMe.getId(), idToken);
                    }
                });
            }
        }
...
//                startActivity(CallScreenActivity.newIntent(this, user, callId, "OUT", CALLTIMEINSEC));
    ...
    private String PROCESSING = "InProcess";
    private String CONVERTED = "Converted";
    ...
    private void StatusSelected() {
        if (statusSelected.equalsIgnoreCase(PENDING)) {
        ...
        } else if (statusSelected.equalsIgnoreCase(FOLLOWUP)) {
            statusSelected = FOLLOWUP;
            ...
            PreferenceConnector.writeString(svContext, PreferenceConnector.ENQSTATUS, FOLLOWUP);
            ...
            txtAllInProcess.setTextColor(getResources().getColor(R.color.black));
            txtAllConverted.setTextColor(getResources().getColor(R.color.black));
        } else if (statusSelected.equalsIgnoreCase(PROCESSING)) {
            statusSelected = PROCESSING;
            txtAllPending.setBackground(getResources().getDrawable(R.drawable.back_left));
            txtAllFollowuP.setBackground(getResources().getDrawable(R.drawable.back_center));
            ...
            PreferenceConnector.writeString(svContext, PreferenceConnector.ENQSTATUS, PROCESSING);
    ...
    private void LoadData() {
        ...
        Toast.makeText(svContext, userID, Toast.LENGTH_LONG).show();
        items = new ArrayList<>();
        RequestQueue queue = Volley.newRequestQueue(this);
...
//                    {"result":true,"message":"data found","data":[{"task_id":"9","appo_id":"3",
            ...
            protected Map<String, String> getParams() {
                ...
                params.put("start_date", txtFrom.getText().toString().trim());
                params.put("end_date", txtTo.getText().toString().trim());
