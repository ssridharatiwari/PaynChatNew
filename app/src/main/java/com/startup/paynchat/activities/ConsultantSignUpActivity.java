...
import static com.startup.paynchat.activities.ActivityPopulateSpinner.defaultSpinnerTitle;
import static com.startup.paynchat.activities.ActivityPopulateSpinner.itemList;
import static com.startup.paynchat.activities.ActivityPopulateSpinner.itemPosition;
import static com.startup.paynchat.activities.ActivityPopulateSpinner.setSpinnerTitle;

public class ConsultantSignUpActivity extends AppCompatActivity {
    private AppCompatSpinner spinnerCountryCodes;
    private EditText etPhone, edName, edMail;
...
//                    view -> {
//                        ActivityCompat.requestPermissions(ConsultantSignInActivity.this, new String[]{Manifest.permission.READ_CONTACTS}, 321);
//                    },
//                    view -> {
//                    });
...
//        } else {
            ...
            setContentView(R.layout.activity_sign_in_consultant);
            ...
            edType.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ActivityPopulateSpinner.showSpinnerData(context, lstRoles);
                }
            });
            ...
            findViewById(R.id.login_txt).setOnClickListener(view -> {
                Intent svIntent = new Intent(ConsultantSignUpActivity.this, SignInActivity.class);
...
//        }
    ...
    private void setupCountryCodes() {
        ...
        if (countries != null) {
            ArrayAdapter<Country> myAdapter = new ArrayAdapter<>(this, R.layout.item_country_spinner, countries);
...
//            toReturn = new ArrayList<>();
    ...
    public void submit() {
        ...
        if (TextUtils.isEmpty(edMail.getText().toString())) {
            Toast.makeText(this, R.string.validation_req_email, Toast.LENGTH_LONG).show();
            return;
        }
        if (TextUtils.isEmpty(edType.getText().toString())) {
            Toast.makeText(this, R.string.validation_req_type, Toast.LENGTH_LONG).show();
            return;
        }
        ...
        builder.setPositiveButton(R.string.yes, (dialogInterface, i) -> {
            ...
            Intent intent = new Intent(ConsultantSignUpActivity.this, VerificationActivityCounsellor.class);
    ...
    private void GetRoles() {
        ...
        StringRequest request = new StringRequest(Request.Method.POST, GlobalVariables.PREURL + GlobalVariables.GETROLES, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Log.d("response===l======>>>>>", response);
                try {
                    lstRoles = new ArrayList<>();
                    JSONObject json = new JSONObject(response);

                    String str_result = json.getString("result");
//                    {"result":true,"message":"data found","data":[{"role":"Lawyer"},{"role":"Counsellor"},{"role":"Psychologist"}]}
                    if (str_result.equalsIgnoreCase("true")) {
                        JSONArray data = json.getJSONArray("data");
                        for (int i = 0; i < data.length(); i++) {
                            JSONObject data_obj = data.getJSONObject(i);
                            lstRoles.add(new SpinnerModel(data_obj.getString("role")));
                        }
                    } else {
    ...
    protected void onResume() {
        super.onResume();
        if (itemPosition != -1 && itemPosition<=itemList.size()){
            setSpinnerTitle(edType, defaultSpinnerTitle);
        }
    }
}
