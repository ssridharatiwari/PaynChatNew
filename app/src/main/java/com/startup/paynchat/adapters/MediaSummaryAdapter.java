...
public class MediaSummaryAdapter extends RecyclerView.Adapter<MediaSummaryAdapter.MyViewHolder> {
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
            ...
            itemView.setOnClickListener(v -> {
                if (file != null && file.exists()) {
                    if (dataList.get(getAdapterPosition()).getAttachmentType() == AttachmentTypes.IMAGE) {
                        context.startActivity(ImageViewerActivity.newMessageInstance(context, dataList.get(getAdapterPosition())));
                    } else {
                        try {
        ...
        public void setData(Message msg) {
            file = Helper.getFile(context, msg, myId);
            switch (msg.getAttachmentType()) {
                case AttachmentTypes.IMAGE:
                    textView.setVisibility(View.GONE);
                    imageView.setBackgroundColor(Color.TRANSPARENT);
                    Glide.with(context).load(msg.getAttachment().getUrl()).apply(new RequestOptions().placeholder(R.drawable.y_placeholder)).into(imageView);
                    break;
                case AttachmentTypes.AUDIO:
                    ...
                    if (file.exists()) {
                        ...
                        try {
                            ...
                            textView.setText(String.format("%02d:%02d", TimeUnit.MILLISECONDS.toMinutes(millis), TimeUnit.MILLISECONDS.toSeconds(millis)));
                            mmr.release();
                        } catch (Exception e) {
                        }
                    }
